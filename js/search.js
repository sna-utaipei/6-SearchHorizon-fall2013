...
{ 
	var request=$.ajax({
		url:"http://searchhorizon.herokuapp.com/search.php",
		dataType:"json"
	}).done(function(data){
		var obj=data,index=0,fuid,fpic,name,content;
			do{
				fuid=obj[index].uid;
				fpic=obj[index].pic;
				name=obj[index].name;
			 	content="<li class='g'><h3 class='fb_name'><a href='http://www.facebook.com/"+fuid+"'>"+name+"</a></h3><img class='fb_pic img-responsive' src='"+fpic+"'/></li>";
				$("div#panel-advanced").append(content);
				index++;
			}while(obj[index]);
			$("div.upgrade").fadeIn();
	}).fail(function(data){
...
function data_fetch(key)
{
	...
	var request=$.ajax({
		...
		if(typeof(obj[1].login) != "undefined")
		{
			...
			{
				var index=0,container="<div id=ires><ol></ol></div>",content;
